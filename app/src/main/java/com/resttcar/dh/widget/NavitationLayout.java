...
public class NavitationLayout extends RelativeLayout{
    ...
    private void setTitles(Context context,List<String> titles, final boolean smoothScroll)
    {
    ...
    public void setNavLine(Activity context, int height, int color, int currentPosition)
    {
        if(textViews != null)
        {
//            navWidth = getScreenWidth(context) / (textViews.length+3);
            navWidth = dip2px(context,74);
        }
        height = dip2px(context,height);
     ...
     */
    public void setViewPager(final Context context, List<String> titles, ViewPager viewPager, final int unselectedcolor, final int setectedcolor, int txtUnselectedSize, final int txtSelectedSize, final int currentPosition, int widOffset, boolean smoothScroll)
    {
