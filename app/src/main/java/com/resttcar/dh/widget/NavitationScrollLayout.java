...
public class NavitationScrollLayout extends RelativeLayout{
    ...
    private void setTitles(Context context,  List<String> titles, final boolean smoothScroll, int splilinecolor, final float splilinewidth, float topoffset, float bottomoffset)
    {
        titleLayout.removeAllViews();
        this.textViews = new TextView[titles.size()];
        ...
        if(textViews != null)
        {
            ...
            for(int i = 0; i < length; i++)
            {
                if(i == position) {
                    ...
                    //textViews[i].setBackgroundResource(R.drawable.item_notice_time_bg);
                }
                else
                {
                    textViews[i].setTextColor(context.getResources().getColor(txtUnselectedColor));
                    textViews[i].setTextSize(txtUnselectedSize);
                   // textViews[i].setBackgroundResource(R.drawable.bg_team_memberinfo);
                }
            }
        }
    }
