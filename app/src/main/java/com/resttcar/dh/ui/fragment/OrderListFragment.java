...
public class OrderListFragment extends BaseFragment {
    ...
    protected void initViews() {
        ...
        adapter.setOnItemClickLitener(new OrderListAdapter.OnItemClickLitener() {
            @Override
            public void onItemClick(View view, int position) {
                OrderDetailsActivity.actionStart(getActivity(), datas.get(position).getOrder_id());
            }
        });
    ...
    List<OrderList.ListBean> datas = new ArrayList<>();

    private void getPurchaseDatas(final int page, String order_no) {

        HttpUtil.createRequest(TAG, BaseUrl.getInstence().orderIndex()).orderList(userToken, page, 10, order_no).enqueue(new Callback<ApiResponse<OrderList>>() {
            @Override
            ...
            @Override
            public void onFailure(Call<ApiResponse<OrderList>> call, Throwable t) {
                svPur.onFinishFreshAndLoad();
